...
| Day 30 | Identity (`is`) vs Equality (`==`) | [View Code](https://github.com) | Explored the crucial difference between identity (`is`) and equality (`==`) operators. Learned that `==` compares the actual values/contents of objects, whereas `is` checks if both variables reference the exact same memory location (object identity). |

| *Day 30* | 'is' vs '=' | [View here tomorrow Code]() | *Next Python topic will be uploaded ! |
